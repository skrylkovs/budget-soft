...
### 1. Фейковый телефон по всему сайту

Захардкожен плейсхолдер `+7 (495) 000-00-00` / `tel:+74950000000`.

> **Уточнение после сверки с кодом:** плейсхолдер не «в kontakty и index.html», а на **всех 27 страницах** (136 вхождений). Источник — **сам генератор** `_generate_pages.py`, 6 мест: строки 1090, 1154, 1165, 1171 (футер + contact-modal, инжектятся в каждую страницу), 1361, 1512. Руками 27 файлов править не нужно — заменить в генераторе и перегнать.

- Заменить на реальный номер РФ во всех вхождениях генератора (и в `tel:`, и в отображаемом тексте, и в `wa.me/`).
- **Нужен ввод от пользователя:** реальный номер телефона.
