...
Бровь + заголовок + текст, затем большой блок с белым фоном: заголовки
`page-prose__heading`, абзацы, а внутри — карточки `page-spec-cards`
(визуально 3 в ширину: Цена + Сроки + Формат оплаты).
...
Бровь + заголовок + текст, затем большая сравнительная таблица `compare compare--service`:
строка-шапка `compare__head` и строки `compare__row`. Колонки — «label» (критерий),
...
Бровь + заголовок + текст, затем от 2 до 6 бенто-блоков, разложенных по рядам
`page-prose-bands__row` (по одному, два или три в ширину). Каждый бенто — карточка
